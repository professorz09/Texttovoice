...
function saveLibraryToStorage(clips: Clip[]) {
  try {
    ...
    if (e instanceof DOMException && e.name === 'QuotaExceededError') {
      alert('Storage quota exceeded! Please delete some clips to free up space.');
    }
  }
}
...
export default function Home() {
  ...
  async function downloadClip(clip: Clip) {
    ...
    a.click();
    toast({ title: "Downloaded!", description: `${clip.title}.wav` });
  }
        ...
        {activeView === "library" && (
          <div className="space-y-3 pb-20">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                <h2 className="font-semibold">Library ({clips.length} clips)</h2>
                <p className="text-xs text-muted-foreground">
                  {(() => {
                    const sizeInMB = new Blob([JSON.stringify(clips)]).size / (1024 * 1024);
                    return `${sizeInMB.toFixed(1)}MB used`;
                  })()}
                </p>
              </div>
